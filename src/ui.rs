...
fn render_dashboard(frame: &mut Frame, area: ratatui::layout::Rect, model: &AppModel) {
    ...
    let profile_items: Vec<ListItem> = if model.store.profiles.is_empty() {
        vec![ListItem::new(
            Span::from("No profiles yet. Go to Profiles screen [2] to create one.").dim(),
        )]
    } else {
        model
            .store
            ...
            .enumerate()
            .map(|(idx, p)| {
                let marker = if idx == model.selected_profile {
                    "▸"
                } else {
                    " "
                };
    ...
    let detail: Vec<Line> = if let Some(profile) = model.store.profiles.get(model.selected_profile)
    {
        ...
        let mut lines = vec![
            Line::from(vec![
                Span::from("Active profile: ").bold(),
                Span::from(&profile.name).cyan().bold(),
            ]),
        ...
        lines.push(Line::from(""));
        lines.push(Line::from(
            Span::from("[i] Install  [2] Profiles  [3] Repositories  [q] Quit").dim(),
        ));
        lines
    } else {
        vec![
...
fn render_profiles(frame: &mut Frame, area: ratatui::layout::Rect, model: &AppModel) {
    ...
    let items: Vec<ListItem> = if model.store.profiles.is_empty() {
        vec![ListItem::new(
            Span::from("No profiles configured. Press [c] to create one.").dim(),
        )]
    } else {
        model
            .store
    ...
    let detail: Vec<Line> = if let Some(profile) = model.store.profiles.get(model.selected_profile)
    {
        ...
        if let Some(ref url) = profile.source_url {
            ...
            lines.push(Line::from(
                Span::from("Source: (uses global sources)").dim(),
            ));
        }
        lines.push(Line::from(""));
        lines.push(Line::from(Span::from("━".repeat(40)).dim()));
        lines.push(Line::from(
            Span::from("[c] Create  [e] Edit  [s] Add skill  [r] Add rule").bold(),
        ));
        lines.push(Line::from(
            Span::from(
                "[X] Remove skill  [Z] Remove rule  [u] Source URL  [d] Duplicate  [x] Delete",
            )
            .bold(),
        ));
        lines.push(Line::from(
            Span::from("[i] Import  [o] Export  [Enter]/[Space] Default  [j/k] Move").dim(),
        ));
        lines
    } else {
    ...
    frame.render_widget(
        Paragraph::new(detail).block(
            Block::default()
                .title(" Profiles — manage profiles ")
                .borders(Borders::ALL)
                .cyan(),
        ),
        right,
...
fn render_repositories(frame: &mut Frame, area: ratatui::layout::Rect, model: &AppModel) {
    ...
    let source_items: Vec<ListItem> = if model.store.sources.is_empty() {
        vec![ListItem::new(
            Span::from("No sources. Press [n] to register a local skill directory.").dim(),
        )]
    } else {
        model
            .store
            ...
            .enumerate()
            .map(|(idx, s)| {
                let marker = if idx == model.selected_source {
                    "▸"
                } else {
                    " "
                };
    ...
    let discoveries: Vec<Line> = if model.discoveries.is_empty() {
        vec![Line::from(
            Span::from("No discoveries yet. Press [r] to scan.").dim(),
        )]
    } else {
        model
            .discoveries
            ...
            .enumerate()
            .map(|(idx, d)| {
                let marker = if idx == model.selected_discovery {
                    "▸"
                } else {
                    " "
                };
                ...
                Line::from(vec![
                    name_span,
                    Span::from(format!("  ({})", d.source_name)).dim(),
                ])
            })
            .collect()
    };
    ...
    lines.push(Line::from(Span::from("━".repeat(40)).dim()));
    lines.push(Line::from(
        Span::from("[n] Add source  [r] Scan  [a]/[Space] Add to profile").bold(),
    ));
    ...
    frame.render_widget(
        Paragraph::new(lines).block(
            Block::default()
                .title(" Repositories — discover skills ")
                .borders(Borders::ALL)
                .cyan(),
        ),
        right,
...
fn render_settings(frame: &mut Frame, area: ratatui::layout::Rect, model: &AppModel) {
    let text = vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::from("Profiles stored: ").bold(),
            Span::from(model.store.profiles.len().to_string()).cyan(),
        ]),
        Line::from(vec![
            Span::from("Sources registered: ").bold(),
            Span::from(model.store.sources.len().to_string()).cyan(),
        ]),
        Line::from(vec![
            Span::from("Default profile: ").bold(),
            Span::from(
                model
                    .store
                    .default_profile_id
...
fn render_about(frame: &mut Frame, area: ratatui::layout::Rect) {
    ...
    let [mascot_area, info_area] =
        Layout::vertical([Constraint::Length(mascot_height), Constraint::Fill(1)]).areas(area);

    let mascot_lines: Vec<Line> = crate::ascii::MASCOT
        .iter()
        .map(|line| {
...
fn render_modal(frame: &mut Frame, area: ratatui::layout::Rect, model: &AppModel) {
    ...
    let mut text = match modal {
        Modal::CreateProfile => format!(
            "Create Profile\n\nName: {}\n\n[Enter] confirm  [Esc] cancel",
            model.input
        ),
        Modal::EditProfile => format!(
            "Edit Profile\n\nName: {}\n\n[Enter] confirm  [Esc] cancel",
            model.input
        ),
        Modal::DeleteProfileConfirm => {
            "Delete selected profile? [Enter] confirm, [Esc] cancel".into()
        }
        Modal::ImportPath => format!(
            "Import Profiles\n\nFile path: {}\n\n[Enter] confirm  [Esc] cancel",
            model.input
        ),
        Modal::ExportPath => format!(
            "Export Profiles\n\nFile path: {}\n\n[Enter] confirm  [Esc] cancel",
            model.input
        ),
        Modal::AddSkill => format!(
            "Add Skill\n\nName: {}\n\n[Enter] confirm  [Esc] cancel",
            model.input
        ),
        Modal::AddRule => format!(
            "Add Rule\n\nRule: {}\n\n[Enter] confirm  [Esc] cancel",
            model.input
        ),
        Modal::RemoveSkill => {
            let skills: String = model
                .store
                .profiles
        ...
        Modal::EditSource => format!(
            "Edit Source URL\n\nZip base URL: {}\n\nExample: https://github.com/user/repo/raw/refs/heads/main/skills\nLeave empty to use global sources.\n\n[Enter] save  [Esc] cancel",
            model.input
        ),
        Modal::InstallPath => format!(
            "Install Profile\n\nTarget: {}\n\n[Enter] install  [Esc] cancel",
            model.input
        ),
        Modal::AddSource => {
            let an = if model.input_focus_secondary {
                ""
            } else {
                " <active>"
            };
            let ap = if model.input_focus_secondary {
                " <active>"
            } else {
                ""
            };
            format!(
                ...
                model.input, model.input_secondary
            )
        }
    };
    ...
    frame.render_widget(
        Paragraph::new(text).block(
            Block::default()
                .title(Line::from(" Modal ").bold().yellow())
                .borders(Borders::ALL)
                .yellow(),
        ),
        area,
...
pub fn render(frame: &mut Frame, model: &AppModel) {
    ...
    let header = vec![
        Line::from(Span::from(" SkillWeaver v0.0.3 ").cyan().bold()),
        Line::from(vec![

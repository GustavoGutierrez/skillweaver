...
fn download_skill(url: &str, skill_name: &str, dest_dir: &Path) -> AppResult<()> {
    let response = ureq::get(url)
        .call()
        .map_err(|e| AppError::Io(std::io::Error::other(format!("dl:{skill_name}:{e}"))))?;
    let mut body = Vec::new();
    response.into_body().as_reader().read_to_end(&mut body)?;
    let tmp_dir = tempfile::tempdir()
        .map_err(|e| AppError::Io(std::io::Error::other(format!("tmp:{skill_name}:{e}"))))?;
    ...
    crate::infra::archive::extract_zip_safely(&tmp_zip, tmp_dir.path())
        .map_err(|e| AppError::Io(std::io::Error::other(format!("extract:{skill_name}:{e}"))))?;
    let skill_inner = tmp_dir.path().join("skills").join(skill_name);
    if dest_dir.exists() {
        std::fs::remove_dir_all(dest_dir)
            .map_err(|e| AppError::Io(std::io::Error::other(format!("rm:{skill_name}:{e}"))))?;
    }
    std::fs::create_dir_all(dest_dir)
        .map_err(|e| AppError::Io(std::io::Error::other(format!("mkdir:{skill_name}:{e}"))))?;
    if skill_inner.is_dir() {
        for entry in std::fs::read_dir(&skill_inner)
            .map_err(|e| AppError::Io(std::io::Error::other(format!("rd:{skill_name}:{e}"))))?
        {
            let entry = entry?;
            ...
            if entry.file_type()?.is_dir() {
                ...
                std::fs::copy(entry.path(), &target).map_err(|e| {
                    AppError::Io(std::io::Error::other(format!("cp:{skill_name}:{e}")))
                })?;
            }
        }
    } else {
...
fn err(ctx: &str, e: impl std::fmt::Display) -> AppError {
    AppError::Io(std::io::Error::other(format!("{ctx}: {e}")))
}
...
pub fn execute_install(
    ...
    if !plan.can_install {
        return Err(AppError::Blocked("preview blocked".into()));
    }

    let skills_dir = project_root.join(".agents/skills");
    if skills_dir.is_file() {
        std::fs::remove_file(&skills_dir).map_err(|e| err("rm_skills_file", e))?;
    }
    std::fs::create_dir_all(&skills_dir).map_err(|e| err("mkdir_skills", e))?;
    ...
    for skill in &profile.skills {
        ...
        let effective_sources: Vec<SourceRegistration> = if let Some(ref url) = profile.source_url {
            vec![SourceRegistration {
                ...
                root: url.clone(),
                source_type: crate::domain::source::SourceType::Zip,
            }]
        } else {
            sources.to_vec()
        };
        ...
        if !found {
            return Err(AppError::Io(std::io::Error::other(format!(
                "no_source for '{skill}' ({} sources available)",
                sources.len()
            ))));
        }
    }
    ...
    apply_managed_rules(&managed, &plan.rules_markdown).map_err(|e| err("agents_md", e))?;
    ...
    save_lockfile(project_root, &lock).map_err(|e| err("lockfile", e))?;

    let has_claude =
        project_root.join("CLAUDE.md").exists() || project_root.join(".claude").is_dir();
    if has_claude {
        let claude_skills = project_root.join(".claude/skills");
        std::fs::create_dir_all(&claude_skills).map_err(|e| err("mkdir_claude", e))?;
        for skill in &profile.skills {
            let src = skills_dir.join(skill);
            let dst = claude_skills.join(skill);
            if src.exists() && !dst.exists() {
                let abs_src = src.canonicalize().unwrap_or_else(|_| src.clone());
                symlink_or_copy(&abs_src, &dst).map_err(|e| err("symlink_{skill}", e))?;
            }
        }
    }

    Ok(if skipped > 0 {
        format!("{downloaded} new, {skipped} ok")
    } else {
        format!("{downloaded} installed")
    })
}
